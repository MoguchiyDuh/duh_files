...
Shared Rust workspace for compiled desktop-rice helpers. Each executable is an
independent crate under `crates/`; the current binaries provide Waybar data that
native modules do not expose well.

## Install

Stow the configuration, build and deploy the shared rice workspace, then run
the optional privileged hardware setup:
...
cd ~/duh_files/rice
./build.py
~/.config/waybar/scripts/setup-hardware.sh
```

`build.py` discovers every binary target through Cargo metadata, performs one
locked release build for the workspace, and atomically installs the results
under `~/.local/bin`. It never requests elevated privileges. Adding a binary
crate under `crates/` automatically includes it in future builds and deploys.

`setup-hardware.sh` grants `hardwarebar` `cap_dac_read_search=ep` when the RAPL
energy counter is root-only. It also uses `dmidecode` once and caches the DIMM
title at
...
and Nerd Fonts. Module actions additionally use `pavucontrol`,
