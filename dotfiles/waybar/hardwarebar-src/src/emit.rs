...
/// rows, a blank separator, then a hint line. Flushed immediately so waybar
/// picks it up as soon as it is written (NDJSON stream, not one-shot exec).
pub fn emit(text: &str, title: &str, fields: &[(&str, String)], hint: &str, classes: &[&str]) {
    let width = fields
        .iter()
...
/// Emit an empty, hidden module line (waybar collapses zero-width text).
pub fn hidden() {
    write_payload("", "", &["hidden"]);
}
